...
class Trading(Link):
    ...
    def __init__(self):
        ...
        self.venue = 'BitMEX - Mr Bath'                         # API key name
    ...
    # MARKET DATA STATE
    def on_startup(self):
		self.fetch_positions(self.venue)
    ...
    def orders_intent(self):
        ...
        bsize = np.clip(self.max_risk - self.risk, 0, self.max_risk) if self.risk < self.max_risk else 0
        asize = np.clip(self.max_risk + self.risk, 0, self.max_risk) if self.risk < self.max_risk else self.max_risk
        ...
        if not any(np.isnan((bid, ask, asize, bsize))):
            ...
            for update in updates:
				try:
					...
					logger.info("cannot amend order "+ json.dumps(update))
